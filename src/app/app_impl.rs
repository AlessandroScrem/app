...
use crate::app::domain::events::{
    AssetEvent, CameraEvent, DomainEvent, EntityEvent, GlobalEvent, SceneEvent, SelectionEvent,
};
...
impl EditorBackend for App {
    ...
    fn command(&mut self, command: EditorCommand, bus: &mut EventBus) -> Vec<EditorEvent> {
        ...
        match command {
            EditorCommand::Select { entities } => {
                let entity = entities.first().copied().map(EntityRawU64::from_raw_u64);
    ...
    fn editor_selection(&self) -> Vec<EntityId> {
        if !self.multiselct.is_empty() {
            self.multiselct.iter().copied().collect()
        } else {
            self.selected.map(|e| e.as_raw_u64()).into_iter().collect()
        }
    }
