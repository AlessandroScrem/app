...
use crate::gpu::{BindgroupLayoutKind, BufferKind, GpuCache, GpuContext, GpuInternalCounters, GpuManager, GpuMaterialCache, GpuMeshCache, GpuSurface, GpuTextureCache, HasGpuStats, IblManager, ShadowManager};
...
impl InternalCounter for Runtime { fn internal_counter(&self) -> GpuInternalCounters { GpuInternalCounters { textures: self.gpu_cache.textures.get_stats(), meshes: self.gpu_cache.mesh.get_stats(), materials: self.gpu_cache.material.get_stats(), shadows: self.shadow_manager.get_stats(), ibl: self.ibl_manager.get_stats() } } }

pub struct Runtime {
    pub window: Arc<Window>, pub gpu_context: GpuContext, pub gpu_surface: GpuSurface, pub gpu_cache: GpuCache, pub gpu_manager: GpuManager, pub ibl_manager: IblManager, pub pipeline_manager: PipelineManager, pub shadow_manager: ShadowManager, pub readback: ReadbackManager, pub uilayer: UiLayer, pub input: Input, pub scene_renderer: SceneRenderer, pub editor_interaction: EditorInteraction, pub imgui_render: ImguiRender, pub hdr_vec: Vec<(TextureId, IblId)>, pub wait_for_exit: bool, pub editor_service: EditorService, last_ui_update: std::time::Instant,
}

impl Runtime {
    pub fn new(window: Arc<Window>) -> Self {
        let mut imgui_context = imgui::Context::create(); let gpu_context = GpuContext::default(); let gpu_surface = GpuSurface::new(gpu_context.adapter(), gpu_context.instance(), window.clone()); let imgui_render = ImguiRender::new(&gpu_context.device, &gpu_context.queue, &window, &mut imgui_context, gpu_surface.get_config().format);
        let gpu_cache = GpuCache { textures: GpuTextureCache::new(&gpu_context.as_ref()), material: GpuMaterialCache::default(), mesh: GpuMeshCache::default() }; let (width, height) = (gpu_surface.get_config().width, gpu_surface.get_config().height); let gpu_manager = GpuManager::new(&gpu_context.as_ref(), width, height); let shadow_manager = ShadowManager::new(&gpu_context.as_ref()); let ibl_manager = IblManager::new(&gpu_context.as_ref()); let pipeline_manager = PipelineManager::new(&gpu_context.device, &gpu_manager, gpu_surface.get_config().format); let scene_renderer = SceneRenderer::new(); let (connection, channels) = EditorConnection::new(); let editor_service = EditorService::new(channels); let uilayer = UiLayer::new(&window, imgui_context, gpu_context.get_adapter_string(), connection);
        Self { window: window.clone(), input: Input::new(), scene_renderer, editor_interaction: EditorInteraction::None, imgui_render, uilayer, gpu_context, gpu_surface, gpu_cache, gpu_manager, ibl_manager, pipeline_manager, shadow_manager, hdr_vec: Vec::new(), wait_for_exit: false, readback: ReadbackManager::default(), editor_service, last_ui_update: std::time::Instant::now() }
    }
    ...
    pub fn handle_input(&mut self, bus: &mut EventBus) {
        ...
        if let Some(result) = self.readback.poll_results() { match result { QueryResult::Pick(id) => bus.send_domain(Selection(Hovered(id.map(Entity::from_raw_u64)))), QueryResult::Selection(ids) => bus.send_domain(Selection(SelectMulti(ids))) } }
        ...
        match self.editor_interaction { EditorInteraction::None => if input.is_mouse_button_pressed(MouseButton::Left) && input.is_key_down(KeyButton::Control) { let current = input.mouse_position; self.editor_interaction = EditorInteraction::Selecting { start: current, current }; }, EditorInteraction::Selecting { start, .. } => { if input.is_mouse_dragging(MouseButton::Left) && input.is_key_down(KeyButton::Control) { self.editor_interaction = EditorInteraction::Selecting { start, current: input.mouse_position }; } if input.is_mouse_button_released(MouseButton::Left) { let current = input.mouse_position; self.editor_interaction = EditorInteraction::None; let pos = (start.x.min(current.x) as u32, start.y.min(current.y) as u32); self.readback.request_selection(&self.gpu_context.as_ref(), &self.gpu_manager.get_framebuffer_texture(crate::gpu::FramebufferKind::EntityId), pos, ((start.x-current.x).abs() as u32, (start.y-current.y).abs() as u32)); } } }
    ...
    pub fn sync_gpu_assets(&mut self, asset_mgr: &mut AssetManager, bus: &mut EventBus) {
        ...
        grouped.process_type::<TextureAsset, _>(|kind, events| match kind { AssetEventKind::Created => { let jobs: Vec<(TextureId, TextureDesc)> = events.iter().filter_map(|ev| asset_mgr.get::<TextureAsset>(ev.id).map(|a| (ev.id, a.desc.clone()))).collect(); for (id, data) in load_cpu_textures_par(jobs) { texture_cache.insert(id, GpuTextureBuilder::from_cpu(data).build(&gpu_context.as_ref())); } }, AssetEventKind::Removed => events.iter().for_each(|ev| texture_cache.remove(ev.id)), _ => {} });
        grouped.process_type::<IblAsset, _>(|kind, events| if let AssetEventKind::Created = kind { events.iter().filter_map(|ev| asset_mgr.get::<IblAsset>(ev.id).map(|a| (ev.id, a))).for_each(|(id, asset)| if let Some(hdr) = texture_cache.get(asset.hrd_id) { ibl_manager.insert(id, ibl_manager.create(hdr, &gpu_context.as_ref())); self.hdr_vec.push((asset.hrd_id, id)); bus.send_domain(Selection(SelectIbl(id))); bus.send_runtime(RuntimeEvent::UpdateIblMaps(id)); }); });
        grouped.process_type::<MaterialAsset, _>(|kind, events| match kind { AssetEventKind::Created => events.iter().filter_map(|ev| asset_mgr.get::<MaterialAsset>(ev.id).map(|a| (ev.id, a))).for_each(|(id, asset)| { let layout = gpu_manager.get_bindgroup_layout(BindgroupLayoutKind::Material); material_cache.insert(id, GpuMaterial::new(&texture_cache, &asset.desc, &gpu_context.device, layout)); }), AssetEventKind::Updated => events.iter().filter_map(|ev| asset_mgr.get::<MaterialAsset>(ev.id).map(|a| (ev.id, &a.desc))).for_each(|(id, desc)| material_cache.update(&id, |m| m.update_uniform(&gpu_context.queue, desc))), AssetEventKind::Removed => events.iter().for_each(|ev| material_cache.remove(ev.id)), _ => {} });
    ...
    pub fn update_ui<A: Application>(&mut self, app: &mut A, bus: &mut EventBus) {
        let now = std::time::Instant::now(); let dt = now.duration_since(self.last_ui_update).as_secs_f32(); self.last_ui_update = now;
        let frame = self.scene_renderer.get_render_stats();
        self.editor_service.set_statistics(EditorStatisticsData { fps: if dt > 0.0 { 1.0 / dt } else { 0.0 }, frametime: dt, adapter_name: self.gpu_context.get_adapter_string(), root_nodes: 0, opaque_draw_calls: frame.opaque.draw_calls, opaque_instances: frame.opaque.instances, transmission_draw_calls: frame.transmission.draw_calls, transmission_instances: frame.transmission.instances });
