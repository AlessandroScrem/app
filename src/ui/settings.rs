...
impl SettingsUi {
    fn draw(&mut self, ui: &Ui, ctx: &mut UiContext, settings: &EditorSettingsData) {
        if ui.collapsing_header("Statistics", TreeNodeFlags::DEFAULT_OPEN) {
            ui.text(format!("FPS: {:.1}", settings.fps));
            ui.text(format!("Frametime: {:.3} ms", settings.frametime * 1000.0));
            ui.text(format!("Root nodes: {}", settings.root_nodes));
            ui.text(format!("Opaque: {} calls | {} instances", settings.opaque_draw_calls, settings.opaque_instances));
            ui.text(format!("Transmission: {} calls | {} instances", settings.transmission_draw_calls, settings.transmission_instances));
            ui.text(format!("Adapter: {}", settings.adapter_name));
        }
        ...
        if ui.collapsing_header("Toggles", TreeNodeFlags::DEFAULT_OPEN) {
            ...
            if settings.bbox_enable {
                toggle(ui, "Box aligned", settings.bbox_axis_aligned, ctx, EditorCommand::SetBoundingBoxAxisAligned);
            }
            ...
            if ui.slider_config("Scene Exposure", 0.001, 64.0).flags(SliderFlags::LOGARITHMIC).build(&mut exposure) {
                ctx.connection.commands.send(EditorCommand::SetExposure(exposure));
            }
            ...
            if ui.slider_config("IBL Intensity", 0.01, 10_000.0).flags(SliderFlags::LOGARITHMIC).build(&mut ibl_intensity) {
                ctx.connection.commands.send(EditorCommand::SetIblIntensity(ibl_intensity));
            }
            let mut env_rotation = settings.env_rotation;
            if ui.slider_config("Env rotation", 0.0, 360.0).build(&mut env_rotation) {
                ctx.connection.commands.send(EditorCommand::SetEnvironmentRotation(env_rotation));
            }
            ...
            if ui.combo("Debug Mode", &mut debug, &DEBUG, |item| std::borrow::Cow::Borrowed(*item)) {
                ctx.connection.commands.send(EditorCommand::SetDebugCode(debug as u32));
            }
            ...
            if ui.combo("Tonemap", &mut tonemap, &TONEMAP, |item| std::borrow::Cow::Borrowed(*item)) {
                ctx.connection.commands.send(EditorCommand::SetTonemap(tonemap as u32));
            }
        ...
        if ui.collapsing_header("Camera", TreeNodeFlags::DEFAULT_OPEN) {
            ...
            if Drag::new("FOV").range(1.0, 179.0).speed(1.0).build(ui, &mut fov) {
                ctx.connection.commands.send(EditorCommand::SetCameraFov(fov));
            }
            let mut distance = settings.camera_distance;
            if Drag::new("Distance").range(0.0, f32::MAX).speed(1.0).build(ui, &mut distance) {
                ctx.connection.commands.send(EditorCommand::SetCameraDistance(distance));
            }
            let mut near = settings.camera_near;
            let mut far = settings.camera_far;
            if Drag::new("Near").range(0.01, f32::MAX).speed(0.1).build(ui, &mut near) {
                ctx.connection.commands.send(EditorCommand::SetCameraNearFar { near, far });
            }
            if Drag::new("Far").range(0.1, f32::MAX).speed(1.0).build(ui, &mut far) {
                ctx.connection.commands.send(EditorCommand::SetCameraNearFar { near, far });
            }
        }
    }
