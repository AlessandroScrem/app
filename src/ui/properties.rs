...
impl UiContext<'_> {
    ...
    pub fn is_editing(&self, entity: EntityId) -> bool { self.edit.as_ref().is_some_and(|edit| edit.key == entity) }
    pub fn begin_edit(&mut self, entity: EntityId, value: EditValue) { *self.edit = Some(EditorEdit::new(entity, value)); }
    pub fn end_edit(&mut self) { *self.edit = None; }
}

impl EditValue {
    pub fn name(&self) -> Option<&str> { match self { Self::Name(value) => Some(value), _ => None } }
    pub fn transform(&self) -> Option<&TransformData> { match self { Self::Transform(value) => Some(value), _ => None } }
    #[allow(dead_code)]
    pub fn light(&self) -> Option<&LightData> { match self { Self::Light(value) => Some(value), _ => None } }
}
...
fn draw_inspector_name(ui: &Ui, ctx: &mut UiContext, inspector: &InspectorData) {
    ...
    if edited {
        ...
        ctx.commands.send(EditorCommand::Entity(EntityCommand::Edit { entity: inspector.entity, edit: EditValue::Name(name.clone()) }));
    }
...
fn draw_inspector_transform(ui: &Ui, ctx: &mut UiContext, inspector: &InspectorData) {
    ...
    let mut transform = ctx.edit(inspector.entity).and_then(EditValue::transform).unwrap_or(&inspector.transform.clone()).to_owned();
    let (edited, activated, deactivated) = ui.group(|| {
        let translation_edited = Drag::new("Translation").speed(0.1).build_array(ui, &mut transform.translation);
        ...
        let rotation_edited = Drag::new("Rotation").speed(0.01).build_array(ui, &mut transform.rotation);
        ...
        let scale_edited = Drag::new("Scale").speed(0.1).build_array(ui, &mut transform.scale);
        ...
        (translation_edited || rotation_edited || scale_edited, translation_activated || rotation_activated || scale_activated, translation_deactivated || rotation_deactivated || scale_deactivated)
    });
    if activated {
        ctx.begin_edit(inspector.entity, EditValue::Transform(transform.clone()));
        ctx.commands.send(EntityCommand::BeginTransformEdit { entity: inspector.entity });
    }
    if edited {
        ctx.begin_edit(inspector.entity, EditValue::Transform(transform.clone()));
        ctx.commands.send(EntityCommand::SetTransform { entity: inspector.entity, transform: transform.clone() });
    }
    if deactivated {
        ctx.commands.send(EntityCommand::EndTransformEdit { entity: inspector.entity });
        ctx.end_edit();
    }
    ui.separator();
    if ui.small_button("Reset Transform") {
        reset_transform(ctx, inspector.entity, TransformData { translation: [0.0; 3], rotation: [0.0; 3], scale: [1.0; 3] });
    }
    ui.same_line();
...
fn reset_transform(ctx: &mut UiContext, entity: EntityId, transform: TransformData) {
    ...
    ctx.commands.send(EntityCommand::SetTransform { entity, transform });
...
fn draw_light(ui: &Ui, ctx: &mut UiContext, entity: u64, source: &LightData) {
    ...
    if edited { ctx.commands.send(EntityCommand::Edit { entity, edit: EditValue::Light(light.clone()) }); }
}
